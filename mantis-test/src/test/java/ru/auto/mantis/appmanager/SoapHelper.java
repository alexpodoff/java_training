...
import ru.auto.mantis.model.Issue;
import ru.auto.mantis.model.Project;
...
public class SoapHelper {
    ...
    public Issue addIssue(Issue issue) throws MalformedURLException, ServiceException, RemoteException {
        ...
        String[] categories = mc.mc_project_get_categories(
                "administrator", "secret", BigInteger.valueOf(issue.getProject().getId()));
        IssueData issueData = new IssueData();
        issueData.setSummary(issue.getSummary());
        issueData.setDescription(issue.getDescription());
        issueData.setProject(new ObjectRef(BigInteger.valueOf(issue.getProject().getId()), issue.getProject().getName()));
        issueData.setCategory(categories[0]);
        BigInteger issueId = mc.mc_issue_add("administrator", "secret", issueData);
        IssueData createdIssueData = mc.mc_issue_get("administrator", "secret", issueId);
